  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (!email || !fullname || !password) {
        return <div>Something went wrong during Signing</div>;
      }
      ...
      <div className="flex items-center justify-center min-h-screen bg-gradient-to-r from-blue-100 to-white">
        <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-md">
          ...
          >
            ...
            <div>
              <Input
                ...
                endIcon={
                  visible ? (
                    <PiEyeBold onClick={() => setVisible(!visible)} />
                  ) : (
                    <PiEyeClosed onClick={() => setVisible(!visible)} />
                  )
                }
              />
              <p className="text-xs font-normal text-red-400 mt-1">
            ...
            <Button
              type="submit"
              className="w-full font-medium text-lg py-2 rounded-md bg-[#154B8B] text-white"
            >
              Login
            </Button>
          </form>
          ...
          <h2 className="text-gray-600 flex gap-2 items-center text-base font-normal mt-6">
            Are you new?{" "}
            <Link to="/sign-up" className="text-[#154B8B] underline">
              Sign up
            </Link>
          </h2>
        </div>
