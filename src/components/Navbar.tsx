...
import Logo from "../new_assets/Logo.webp";
import languageicon from "../new_assets/languageicon.webp";
...
const Navbar = () => {
  ...
  const menuRef = useRef<HTMLUListElement>(null); // Specifying the type here
  ...
  useMotionValueEvent(scrollY, "change", (latest) => {
    const prev = scrollY.getPrevious() ?? 0; // Handle possibly undefined previous value
        ...
        <div className="px-5 py-5 w-[90vw] md:w-[80%] sm:w-full bg-white rounded-xl justify-between items-center">
          <menu className="hidden md:block items-center justify-between object-cover" style={{ width: "100%" }}>
            <ul className="items-center w-full" style={{ display: "flex", flexDirection: 'row', justifyContent: "space-around" }}>
              ...
              <li>
                <NavLink className={({ isActive }) => isActive ? "text-black" : "text-slate-600"} to="chatbot"> <span className="hover:text-black">{t('navbar.AI Chatbot')}</span></NavLink>
              </li>
              <li>
              ...
              <li className="relative cursor-pointer group">
                ...
                <div className="absolute z-[9999] hidden group-hover:block w-[200px] rounded-md bg-gray-100 shadow-md p-2 dark:text-white">
                  <ul className="space-y-2">
                    <li className="text-gray-500 hover:text-gray-900 duration-200 inline-block w-full p-2 hover:bg-gray-400 rounded-md font-semibold" onClick={() => changeLanguage('en')}>
                      English
                    </li>
                    <li className="text-gray-500 hover:text-gray-900 duration-200 inline-block w-full p-2 hover:bg-white rounded-md" onClick={() => changeLanguage('hi')}>
                      हिंदी
                    </li>
                  </ul>
                </div>
          ...
          <button className="md:hidden w-full flex justify-between items-center z-60" onClick={() => setOpen(!open)}>
            <div className="h-[0] flex items-center"><img className="h-[70px] w-[100px]" src={Logo} alt="adiray" /></div>
            {open ? <AiOutlineClose className="relative z-70" style={{ fontSize: "23px" }} /> : <FiMenu style={{ fontSize: "20px" }} />}
          </button>
        </div>
        {open && (
          <ul ref={menuRef} data-aos={open ? "slide-left" : "slide-right"} className={`p-6 md:hidden absolute top-24 right-0 w-[50%] flex flex-col justify-between items-center gap-8 bg-[#E8EDF3] rounded-md transition-all duration-300 ease-in-out`}>
            <button className="md:hidden flex transition-margin duration-300 ease-in-out" onClick={() => openNav(open)}></button>
            <li>
            ...
            <li className="relative cursor-pointer group">
              ...
              <div className="absolute z-[9999] hidden group-hover:block w-[200px] rounded-md bg-[#E8EDF3] shadow-md p-2 dark:text-white">
                <ul className="space-y-2">
                  <li className="text-gray-500 hover:text-gray-500 duration-200 inline-block w-full p-2 hover:bg-pink-200 rounded-md font-semibold" onClick={() => changeLanguage('en')}>
                    English
                  </li>
                  <li className="text-gray-500 hover:text-gray-500 duration-200 inline-block w-full p-2 hover:bg-pink-200 rounded-md" onClick={() => changeLanguage('hi')}>
                    हिंदी
                  </li>
                </ul>
              </div>
